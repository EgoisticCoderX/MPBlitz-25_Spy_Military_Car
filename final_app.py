...
latest_detections = []
# Add global gun_detected, ESP32 URL, and ESP32 busy flag
esp32_url = "http://192.168.42.164"
gun_detected = False
    ...
    def detect_objects(self, image):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                global gun_detected
                with detection_lock:
                    prev_gun_detected = gun_detected
                    gun_detected = any(
                        det.get('class', '').lower() in ['gun', 'weapon']
                        for det in predictions.get('predictions', [])
                    )
                # Sync gun detection state to ESP32 if changed
                try:
                    if gun_detected != prev_gun_detected:
                        requests.post(f"{esp32_url}/set_gun_status", data={"detected": str(gun_detected).lower()}, timeout=1)
                except Exception as e:
                    print(f"ESP32 sync error: {e}")
...
def status():
    """
    Get the status of gun detection
    """
    with detection_lock:
...
# ESP32 status endpoint for frontend to check busy state
@app.route('/esp32_status')
def esp32_status():
    with detection_lock:
        return jsonify({'esp32_busy': esp32_busy})

# Robust ESP32 trigger: block if busy, add cooldown, thread-safe
@app.route('/toggle_esp32', methods=['POST'])
def toggle_esp32():
    global esp32_busy
    with detection_lock:
        if not gun_detected:
            return jsonify({'success': False, 'error': 'Gun not detected'})
        if esp32_busy:
            return jsonify({'success': False, 'error': 'ESP32 action already in progress'})
        esp32_busy = True
    try:
        response = requests.post(f'{esp32_url}/toggle_led', timeout=5)
        if response.status_code == 200:
            # Add a short cooldown to prevent double trigger (e.g., 1.5s)
            def release_busy():
                global esp32_busy
                time.sleep(1.5)
                with detection_lock:
                    esp32_busy = False
            threading.Thread(target=release_busy, daemon=True).start()
            return jsonify({'success': True})
        else:
            with detection_lock:
                esp32_busy = False
            return jsonify({'success': False, 'error': 'ESP32 request failed'})
    except Exception as e:
        with detection_lock:
            esp32_busy = False

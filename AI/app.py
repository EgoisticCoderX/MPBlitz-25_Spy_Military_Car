...
if not ROBOFLOW_API_KEY or not ESP32_CAM_URL:
    raise ValueError("FATAL ERROR: Please set ROBOFLOW_API_KEY and ESP32_CAM_URL in your .env file.")

# Create directories for static files and captures on startup
os.makedirs("static/captures", exist_ok=True)

# Confidence thresholds - start here and tune if necessary
DETECTION_MODELS = {
    "soldier_detection": {"name": "Soldier", "models": [{"id": "civil-soldier/1", "weight": 0.4}, {"id": "soldier-ijybv-wnxqu/1", "weight": 0.6}], "conf": 0.60, "color": (0, 255, 0)},
    "gun_detection": {"name": "Gun", "models": [{"id": "weapon-detection-ssvfk/1", "weight": 0.5}, {"id": "gun-d8mga/2", "weight": 0.5}], "conf": 0.65, "color": (128, 0, 128)}
}

app = Flask(__name__)

# --- CORE STREAMING AND DETECTION CLASS ---
class ESP32CamStream:
    ...
    def _stream_loop(self):
        try:
            ...
            print(f"ERROR: Stream disconnected. Reason: {e}")
        finally:
        ...
        while self.running:
            ...
            with self.state_lock:
                if self.state == "STREAMING_LIVE" and detected_classes:
                    ...
                    # NEW FEATURE: Save the detection image
    ...
    def run_detection_on_frame(self, frame):
        ...
        models_to_run = {}
        if self.active_model == "all":
            models_to_run = DETECTION_MODELS
        elif self.active_model in DETECTION_MODELS:
            models_to_run = {self.active_model: DETECTION_MODELS[self.active_model]}
...
def set_brightness_route():
    ...
    try:
        ...
        return jsonify(status="success")
    except Exception as e: return jsonify(status="error", message="ESP32 not responding"), 502
